...
verify = :crypto.verify(:ecdsa,:sha256,Transaction.makestring(transaction),transaction.signature|>Base.decode16!,[publickey |> Base.decode16! , :secp256k1])
end
...
mainblockchain=cond do
                wallet1.balance >= sendamount -> createtrans(wallet1,wallet2address,sendamount,mainblockchain)
...
def createtrans(wallet1,wallet2address,sendamount,mainblockchain) do
 transaction1   = %Transaction{from: wallet1.publickey ,to: wallet2address  ,amount: sendamount , timestamp: System.system_time(:second)}
...
blocksum = Enum.reduce(transactlist,0,fn(x,acc) ->
  cond do
