...
input = Integer.to_string(block.index) <> block.previousHash <>  Integer.to_string(block.timestamp) <> trans <> Integer.to_string(block.nonce)
...
%Block{ index: latest_block.index + 1,previousHash: latest_block.hash,timestamp: System.system_time(:second),data: datafield,nonce: newnonce}
|> calchash
end
...
def genblock do
transaction = %Transaction{from: "Genesis_sender" ,to: "Genesis_destination"  ,amount: 0, timestamp: System.system_time(:second)}
%Block{ index: 0,previousHash: "",timestamp: System.system_time(:second),data: [transaction],nonce: 0}
|> calchash
end
